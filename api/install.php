...
if ($conn->connect_error) {
    ...
    if ($db_selected) {
        ...
        $queries['assignments'] = "CREATE TABLE IF NOT EXISTS `assignments` (
            `id` INT AUTO_INCREMENT PRIMARY KEY,
        ...
        if ($success) {
            $seed_username = 'admin';
            $seed_password = 'VidiVeda@2026';
            $admin_pass_hash = password_hash($seed_password, PASSWORD_DEFAULT);
            
            $check_admin = $conn->prepare("SELECT id FROM admin_users WHERE username = ?");
            $check_admin->bind_param("s", $seed_username);
            $check_admin->execute();
            $check_admin->store_result();
            
            if ($check_admin->num_rows === 0) {
                $insert_admin = $conn->prepare("INSERT INTO admin_users (username, password_hash) VALUES (?, ?)");
                $insert_admin->bind_param("ss", $seed_username, $admin_pass_hash);
        ...
        if ($success) {
            ...
            $message = "Database tables and administrator account initialized successfully!<br><br>
                       ...
                       <strong>Default Admin Username:</strong> <code>admin</code><br>
                       <strong>Default Admin Password:</strong> <code>VidiVeda@2026</code><br>
                       <em>These are stored in the <code>admin_users</code> table. Change them after first login.</em><br><br>
